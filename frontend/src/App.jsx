...
function HomePage() {
  return (
    <div
      ...
      style={{ background: "linear-gradient(135deg,#EEF7FF 0%,#F5FAFF 50%,#F7F3FF 100%)" }}
    >
      <header className="w-full sticky top-0 z-10 bg-white/70 backdrop-blur border-b border-slate-200">
        <div className="px-[clamp(12px,2vw,24px)] py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-lg font-semibold text-slate-800">
            <div className="rounded-full border border-sky-400 px-2 py-1 text-sky-600 font-bold">FB</div>
            Farmland Boundary Detection System
          </div>
          <nav className="space-x-6 text-sm font-medium text-sky-700">
            <a href="#/" className="hover:text-slate-900">Home</a>
            <a href="#/use" className="hover:text-emerald-600">Use</a>
          </nav>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center w-full">
          <div>
            ...
            <p className="mt-4 text-slate-600 text-sm leading-relaxed">
              Upload imagery or specify a region of interest. The pipeline runs segmentation,
              postprocessing, and visualization — giving you clean farmland boundaries.
            </p>
...
function UsePage() {
  ...
  // 使用页卡片布局
  const leftCardMinH  = tab === "coords" ? "min(60vh, 640px)" : "min(49vh, 500px)";  
  const rightCardMinH = tab === "coords" ? "min(56vh, 600px)" : "min(45vh, 480px)";  
  const uploadPreviewH = tab === "coords" ? "min(34vh, 340px)" : "min(27vh, 280px)"; 
  const resultPreviewH = tab === "coords" ? "min(34vh, 340px)" : "min(27vh, 280px)"; 
  ...
  async function handleDownload() {
    ...
    if (jobId) {
      try {
        const resp = await fetch(ENDPOINTS.jobDownload(jobId), { method: "GET" });
    ...
    >
      ...
      <main className="flex-1 w-full px-[clamp(12px,2vw,24px)] py-10">
        ...
        <p className="mt-2 text-slate-700 text-sm">
          Input coordinates or upload imagery. The backend will process and show a preview.
        </p>
          ...
          >
                ...
                >
                  {uploadPreviewUrl ? (
                    ...
                    <div className="h-full w-full flex flex-col items-center justify-center text-slate-500">
                      <p className="text-sm">
                        Drag &amp; drop image (<span className="font-medium">GeoTIFF / JPEG / PNG</span>), or
                      </p>
                      <button
                        type="button"
            ...
            <div className="mt-6">
              <div className="text-sm text-slate-800 mb-2">
                Status: <span className={status === "done" ? "text-green-600" : status === "running" ? "text-sky-600" : "text-slate-500"}>
                  {status === "idle" ? "None" : status === "running" ? "Running..." : "Done"}
                </span>
              </div>
          ...
          >
            ...
            <div className="flex justify-center mt-4">
              <button
                ...
                title={canDownload ? (gpkgName || "result.gpkg") : "Result not ready"}
